...
class SkinnyBaseControllerCommonCrmFree {
   ...
   */
    public function main()
    {
           ...
           // Slash after the module missing?
            ...
            if (  $hasMissingSlash  ) {
                ...
                if (  $this->allowModulesAsFiles   ) {
                    ...
                    echo file_get_contents(WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY."templates/404.php");
                    exit;
                }
            }
        ...
        $this->require_once_many(WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY."lib/skinnymvc/core/base/*.php");
        $this->require_once_many(WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY."lib/skinnymvc/core/*.php");
		
        // Get the db controller classes
        $this->require_once_many(WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY."lib/skinnymvc/dbcontroller/base/*.php");
        $this->require_once_many(WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY."lib/skinnymvc/dbcontroller/*.php");
			
        // Get the controller classes
        $this->require_once_many(WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY."lib/skinnymvc/controller/base/*.php");
        $this->require_once_many(WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY."lib/skinnymvc/controller/*.php");
			
        //Get all Model classes
        if (SkinnySettingsCommonCrmFree::$CONFIG['preload model']) {
            $this->require_once_many(WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY."lib/skinnymvc/model/*.php");
            $this->require_once_many(WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY."lib/skinnymvc/model/base/*.php");
        }

        //Initialize session
        if (SkinnySettingsCommonCrmFree::$CONFIG['session persistency']) {
            $this->skinnyUser = SkinnyUser::getUser();
        }

        //Get all plugins
        $this->require_once_many(WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY."plugins/skinnyPlugin*.php");
    ...
    {
        if (!file_exists(WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY."modules/$module/actions/actions.php")) {
            //Error: Action does not exist
            header("HTTP/1.1 404 Not Found");
            echo file_get_contents(WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY."templates/404.php");
            exit;
        }
        require_once(WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY."modules/$module/actions/actions.php");
        ...
        if ( $moduleObj->allowUndefinedActions()) {
            ...
            if (  is_callable(array($moduleObj, $actionMethod))  ) {
                ...
                if (!file_exists(WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY."modules/$module/templates/$action.php")) {
                    //Error: Action does not exist
                    header("HTTP/1.1 404 Not Found");
                    echo file_get_contents(WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY."templates/404.php");
                    exit;
            ...
            if (SkinnySettingsCommonCrmFree::$CONFIG['debug']) {
                global $__DEBUG;
        ...
        $actionTemplateSource = file_get_contents(WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY."modules/$module/templates/$action.php");
    ...
    protected function checkAuthentication($moduleObj, $skinnyUser, $param) {
        ...
        if ($moduleObj->authenticatedOnly()) {
            if (!$skinnyUser->isAuthenticated()) {
                //Not authenticated!
                if (isset(SkinnySettingsCommonCrmFree::$CONFIG['unauthenticated default module'])) {
                    if (isset(SkinnySettingsCommonCrmFree::$CONFIG['unauthenticated default action'])) {
                        $moduleObj->redirect(SkinnySettingsCommonCrmFree::$CONFIG['unauthenticated default module'], SkinnySettingsCommonCrmFree::$CONFIG['unauthenticated default action'], $param);
                    } else {
                        $moduleObj->redirect(SkinnySettingsCommonCrmFree::$CONFIG['unauthenticated default module'], "index", $param);
                    }
                } else {
   ...
   */
   ...
   private function processLayout(&$skinny_content, $layoutData, $skinnyUser, $module, $action, $layout) {
      include_once WP_CONST_ULTIMATE_CRM_CPT_DIRECTORY.'templates/'. $layout .'.php';
   }
